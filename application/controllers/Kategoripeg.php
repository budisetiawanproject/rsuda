...
error_reporting(0);
class kategoripeg extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if (empty($data['user'])) {
            ...
            $this->load->view('kategoripeg/index');
    ...
    public function tambah()
    {
        ...
        if ($data['user']['us_username'] == '') {
            ...
            $w = date('YmdHis');
            $kode = 'KP' . $w;
            $kp = $this->input->post('nama');
            $input = [
                'kp_id' => $kode,
                'kp_nama' => $kp,
            ];
            $this->db->set($input);
            $this->db->insert('t_kategori_pegawai');
            $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">
                            Data Berhasil Tambah
                            </div>');
            redirect('kategoripeg');
        }
    }
    ...
    public function edit()
    {
        ...
        if ($data['user']['us_username'] == '') {
            ...
            $this->db->set($update);
            $this->db->where(['kp_id' => $id], 't_kategori_pegawai');
            $this->db->update('t_kategori_pegawai');
