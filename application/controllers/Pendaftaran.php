...
class Pendaftaran extends CI_Controller
{
        ...
        if (empty($data['user'])) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $data = [
                    ...
                    'pas_pj_nama' => htmlspecialchars($this->input->post('namapj')),
    ...
    public function editpasien()
    {
        ...
        if (empty($data['user'])) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $this->load->view('templates/header');
                $this->load->view('templates/sidebar', $data);
                $this->load->view('pendaftaran/editpasien');
                $this->load->view('templates/footertable');
            } else {
                //echo 'tidak jalan';
                if ($provnama) {
                    ...
                    $this->db->set($update);
                    $this->db->where(['pas_id' => $id], 't_pasien');
                    $this->db->update('t_pasien');
                    $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">
    ...
    public function delpasien()
    {
        ...
        if ($data['user']['us_username'] == '') {
            ...
            if ($data['user']['us_role_id'] == '2') {
                $kode = $this->input->post('kode');
                $cek = $this->db->query("SELECT * FROM t_registrasi WHERE pas_id = '$kode'")->row_array();
                if ($cek) {
                    ...
                    $this->db->where(['pas_id' => $kode], 't_pasien');
    ...
    public function register()
    {
        ...
        if (empty($data['user'])) {
            ...
            $this->load->view('templates/header');
            $this->load->view('templates/sidebar', $data);
            $this->load->view('pendaftaran/register');
            $this->load->view('templates/footertable');
        }
    }
